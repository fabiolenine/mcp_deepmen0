...
- `vault/` — DeepMem0 Vault: bearer-token auth for this server plus the admin UI on :8080
  - `store.py` — users/tokens/audit_log in SQLite WAL, schema v1 via `PRAGMA user_version`; every mutation is one `BEGIN IMMEDIATE` that includes its audit row; unique partial index on `renewed_from` (one successor per token); `verify_token()` + conditional `last_used_at` touch. **stdlib only**
  ...
  - `security.py` / `i18n.py` / `web.py` / `main.py` — token+password+email validation, PT/EN strings, Starlette+Jinja2+HTMX UI, `deepmem0-vault` entry point with `bootstrap-admin`. Need the `[vault]` extra
